...
directly above it – X and Y axes match the tag's base-frame position,
Z stays at a fixed hover height, and the TCP orientation is forced to
point straight down (tool Z anti-parallel to base Z).
...
# Fixed "straight-down" orientation in axis-angle (Rodrigues vector).
# Tool Z pointing in the −Z direction of the base frame:
#   rx = π  (180° rotation about X)  → tool Z points down
#   ry = 0
#   rz = 0
# Adjust rz if you need the gripper jaws rotated to a specific angle.
HOVER_RX = 2.225
HOVER_RY = 2.170
HOVER_RZ = 0.022

# Known calibration Y-offset error (add -0.05 m to cancel +50 mm bias)
CALIB_Y_OFFSET_M = -0.050   # subtract 50 mm from the detected Y
CALIB_X_OFFSET_M = -0.005   # subtract 10 mm from the detected X
...
while True:
    ...
    if ids is not None:
        ...
        for i, mid in enumerate(ids.flatten()):
            ...
            # Project tag origin onto image for visual overlay
            ...
            # Transform tag position → base frame
            ...
            tag_pos_base[0] += CALIB_X_OFFSET_M   # apply known X calibration correction
            tag_pos_base[1] += CALIB_Y_OFFSET_M   # apply known Y calibration correction
            tvec_cam     = tvec
            tag_detected = True
    ...
    cv2.imshow("15_hover", cv2.resize(frame, (960, 540)))
    ...
    if key == ord('q'):
        ...
        bx, by, _ = tag_pos_base   # use tag X, Y; ignore detected Z

        # Build the hover pose: tag X, Y  +  fixed absolute Z  +  straight-down orientation
        ...
        print("\n" + "=" * 60)
        print(f"  Tag in base frame:     X={bx:.4f}  Y={by:.4f}  (Y corrected {CALIB_Y_OFFSET_M*1000:+.0f}mm)")
        print(f"  Hover Z (fixed):       {HOVER_Z_M:.4f} m")
        print(f"  Target pose:           {[round(v, 4) for v in target_pose]}")
        print(f"  Distance to target:    {dist_mm:.1f} mm")
        print(f"  Orientation (radians): RX={HOVER_RX:.4f}  RY={HOVER_RY:.4f}  RZ={HOVER_RZ:.4f}")
        print("=" * 60)
